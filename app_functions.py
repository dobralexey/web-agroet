...
from app_translation import TRANSLATIONS, BAND_NAMES
...
def render_timeseries_plot(df, meteo_df):
    ...
    if 'ETc_std' in df.columns:
        ax.fill_between(df['date'], df['ETc'] - df['ETc_std'], df['ETc'] + df['ETc_std'], color=colors['ETc'],
                        alpha=0.2)
    if 'ETs_std' in df.columns:
        ax.fill_between(df['date'], df['ETs'] - df['ETs_std'], df['ETs'] + df['ETs_std'], color=colors['ETs'],
                        alpha=0.2)
    if 'ET' in df.columns:
    ...
    ax.plot(meteo_df['Date'], meteo_df['ETr_24_ERA'], color=colors['ERA'], linewidth=1, linestyle='--', alpha=0.9,
            label='_nolegend_')
...
def create_map_with_cropped_tiff(tiff_path, band_idx=19, shape_gdf=None):
    ...
    folium.TileLayer(tiles='https://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}', attr='Google', name='Google Hybrid',
    ...
    if len(valid_data) > 0:
        colormap_legend = cm.LinearColormap(colors=reversed_colors, vmin=band_min, vmax=band_max, text_color='white',
                                            caption='')
        m.add_child(colormap_legend)
    image_overlay = folium.raster_layers.ImageOverlay(image=f'data:image/png;base64,{img_base64}', bounds=bounds_4326,
                                                      opacity=0.85, name=f'Band {band_idx} Overlay', interactive=True,
                                                      cross_origin=False, zindex=2)
    image_overlay.add_to(m)
    folium.GeoJson(shape_gdf_4326, name='Area Boundary',
                   style_function=lambda x: {'fillColor': '#000000', 'color': '#ffffff', 'weight': 2,
                                             'fillOpacity': 0.0, 'dashArray': '5, 5'}).add_to(m)
...
def results_section(tiff_folder, shape_file):
    ...
    selected_date_idx = st.sidebar.selectbox(_("select_date"), range(len(date_options)),
                                             format_func=lambda x: date_options[x], index=len(date_options) - 1)
    ...
    band_options = [f"Band {i + 1} - {BAND_NAMES[i + 1]}" if (i + 1) in BAND_NAMES else f"Band {i + 1}" for i in
                    range(num_bands)]
    selected_band_idx = st.sidebar.selectbox(_("select_band"), range(len(band_options)),
                                             format_func=lambda x: band_options[x],
                                             index=min(default_band_idx, num_bands - 1))
    ...
    st.subheader(
        _("map_subheader_date", date=date_options[selected_date_idx], band_label=band_label, band=selected_band))
    try:
        with st.spinner(_("cropping_spinner")):
            folium_map, valid_data = create_map_with_cropped_tiff(tiff_path=selected_tiff, band_idx=selected_band,
                                                                  shape_gdf=shape_gdf)
            st_folium(folium_map, width=None, height=400, returned_objects=[],
                      key=f"result_map_{selected_date_idx}_{selected_band_idx}")
