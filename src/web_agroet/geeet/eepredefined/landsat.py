...
def add_lai_loglinear(img:ee.Image)->ee.Image:
    ...
    * "log-linear" method (e.g., Fisher et al., 2008):
    ...
    """
    from src.web_agroet.geeet.vegetation import compute_lai
...
def add_lai_houborg2018(img:ee.Image)->ee.Image:
    ...
    ** "houborg2018" multi-vegetation index method (Houborg et al., 2018):
    ...
    Note: LAI is clamped to (0,7)
    """
    from src.web_agroet.geeet.vegetation import lai_houborg2018
    band_names=['SR_B2', 'SR_B4', 'SR_B5', 'SR_B6', 'SR_B7']
    bands_l7=['SR_B1', 'SR_B3', 'SR_B4', 'SR_B5', 'SR_B7']
    spacecraft_id = ee.String(img.get('SPACECRAFT_ID'))
    sel_bands = ee.Algorithms.If(spacecraft_id.equals('LANDSAT_7'),bands_l7, band_names)
...
def collection(
    ...
    """
    import ee
    from .join import landsat_ecmwf, landsat_ecmwf_interp
    ...
    if era5:
        ...
        meteo_bands = src.web_agroet.geeet.eepredefined.MeteoBands.ECMWF_ERA5_HOURLY_TSEB
        meteo_prep = src.web_agroet.geeet.eepredefined.MeteoPrep.ECMWF_ERA5_HOURLY_TSEB
        ...
        meteo_collection = (ee.ImageCollection('ECMWF/ERA5_LAND/HOURLY')
            ...
            .map(meteo_prep))
        if era5_interp:
            collection = landsat_ecmwf_interp(
...
def tseb_series(**kwargs)->Callable:
    ...
    """
    from src.web_agroet.geeet.tseb import tseb_series as t
    def f(img):
        return t(img, **kwargs)
    return f
...
def extrapolate_LE(img):
    ...
    """
    R = src.web_agroet.geeet.solar.rad_ratio(img)
    LE = img.select("LE")
