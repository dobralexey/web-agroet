...
from src.web_agroet.geeet.common import is_img, is_eenum
...
def compute_solar_angles(doy = None, time = None, longitude = None, latitude = None):
    ...
    '''
    import numpy as np 
    from src.web_agroet.geeet.common import is_eenum
    ...
    if is_eenum(doy):
        ...
        cos_incidence_angle =  sin_solar.multiply(sin_lat).add(cos_solar.multiply(cos_lat).multiply(cos_hour_angle))

        solar_angles = zenith.addBands(azimuth).addBands(cos_incidence_angle).rename(
            ['zenith', 'azimuth', 'cos_incidence'])
...
def compute_d2(doy):
    ...
    import numpy as np
    doy = Number(doy)
    d2 = Number(1).divide(Number(1).add(Number(0.033).multiply((doy.multiply(Number(2*np.pi).divide(Number(365)))).cos())))
    return d2

def compute_tsw(P, W, cos_incidence, Kt=1.0):
    from ee import Image
    Kt = Image(Kt)
    term1 = Image(-0.00146).multiply(P).divide(Kt.multiply(cos_incidence))
    term2 = Image(0.075).multiply((W.divide(cos_incidence)).pow(0.4))
    tsw = Image(0.35).add(Image(0.627).multiply((term1.subtract(term2)).exp()))
    return tsw
...
def compute_Rs(cos_incidence, tsw, d2):
    Gsc = 1367
    from ee import Image
    Rs = Image(Gsc).multiply(cos_incidence).multiply(tsw).divide(d2)
    Rs = Rs.rename("Rs")
    return Rs

def compute_Rlu(Tr_K, fc, EmisVeg = 0.98, EmisGrd = 0.93):
    from ee import Image
    sb = 5.670373e-8
    EmisSurf = (fc.multiply(EmisVeg)).add((Image(1).subtract(fc)).multiply(EmisGrd))
    Rlu = EmisSurf.multiply(sb).multiply(Tr_K.pow(4))
    return Rlu

def compute_emis_atm(tsw):
    from ee import Image
    neg_ln_tau = tsw.log().multiply(-1)
    emis_atm = Image(0.85).multiply(neg_ln_tau.pow(0.09))
    return emis_atm

def compute_Rld(Ta_K, emis_atm):
    sb = 5.670373e-8
    Rld = emis_atm.multiply(sb).multiply(Ta_K.pow(4))
    return Rld


def compute_emis_0(LAI):
    from ee import Image
    emis_0 = Image(0.95).add(Image(0.01).multiply(LAI))
    return emis_0.clamp(0.95, 0.98)

def compute_Rln(Rlu, Rld, emis_0):
    from ee import Image
    Rln = (Rld.subtract(Rlu).subtract((Image(1).subtract(emis_0)).multiply(Rld)))
    return Rln
