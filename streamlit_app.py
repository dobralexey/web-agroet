...
TRANSLATIONS = {
    "en": {
        ...
        "out_ptjpl_label": "Output folder for Landsat PT-JPL images",
        "out_ptjpl_help": "Path to save PT-JPL output images",
        "out_meteo_label": "Output path for ERA5-Land meteorological file",
        "out_meteo_help": "Path to save meteorological data (feather format)",
        ...
        "success_message": "Processing completed!\n\n**Outputs:**\n- PT-JPL images saved to: `{out_ptjpl}`\n- Meteorological data saved to: `{out_meteo}`",
        "error_processing": "❌ Error during processing: ",
        "sidebar_instructions": "ℹ️ Instructions",
        "sidebar_steps": """
        1. **Select area input method** - either upload a shapefile or draw a polygon on the map
        2. **Choose date range** for the analysis
        3. **Select method** (ML PT-JPL or Original PT-JPL)
        4. **Set maximum cloud cover** percentage
        5. **Specify output paths** for results
        6. **Click 'Run PT-JPL Model'** to start processing
        """,
    ...
    "ru": {
        ...
        "output_locations": "💾 Пути сохранения результатов",
        "out_ptjpl_label": "Папка для сохранения снимков Landsat PT-JPL",
        "out_ptjpl_help": "Путь для сохранения выходных изображений PT-JPL",
        "out_meteo_label": "Путь для сохранения метеорологического файла ERA5-Land",
        "out_meteo_help": "Путь для сохранения метеоданных (формат feather)",
        ...
        "success_message": "Обработка завершена!\n\n**Выходные данные:**\n- Изображения PT-JPL сохранены в: `{out_ptjpl}`\n- Метеоданные сохранены в: `{out_meteo}`",
        "error_processing": "❌ Ошибка во время обработки: ",
        "sidebar_instructions": "ℹ️ Инструкция",
        "sidebar_steps": """
        1. **Выберите способ задания области** – загрузите shapefile или нарисуйте полигон на карте
        2. **Выберите диапазон дат** для анализа
        3. **Выберите метод** (ML PT-JPL или оригинальный PT-JPL)
        4. **Установите максимальный процент облачности**
        5. **Укажите пути для сохранения результатов**
        6. **Нажмите «Запустить модель PT-JPL»**, чтобы начать обработку
        """,
...
def get_tif_value(tif_path, shape_gdf):
    with rasterio.open(tif_path) as src:
        ...
        out_image, _mask = mask(src, [mapping(geom)], crop=True, nodata=np.nan)  # FIXED: renamed _ to _mask
        values = []
    ...
    try:
        ...
        with st.expander(_("file_info")):
            with rasterio.open(selected_tiff) as src:
                ...
                cropped_data, _transform, bounds = crop_tiff_to_shape(selected_tiff, shape_gdf)  # FIXED: renamed _ to _transform
...
def main():
    ...
    with st.sidebar:
        lang = st.selectbox(_("language_selector"), options=["en", "ru"], format_func=lambda x: TRANSLATIONS[x]["en" if x=="en" else "ru"], index=0 if get_language() == "en" else 1, key="language_selector_widget")
    ...
    with col1:
        ...
        with col_date1:
            start_date = st.date_input(_("start_date"), value=date(2024, 7, 1), min_value=date(2013, 1, 1), max_value=date(2050, 12, 31))
        with col_date2:
            end_date = st.date_input(_("end_date"), value=date(2024, 7, 31), min_value=date(2013, 1, 1), max_value=date(2050, 12, 31))
        ...
        st.subheader(_("output_locations"))
        out_ptjpl = st.text_input(_("out_ptjpl_label"), value='./data/images/', help=_("out_ptjpl_help"))
        out_meteo = st.text_input(_("out_meteo_label"), value='./data/meteo/meteo.feather', help=_("out_meteo_help"))
    with col2:
        ...
        config_data = {
            ...
            _("config_out_ptjpl"): out_ptjpl,
            _("config_out_meteo"): out_meteo,
        }
        ...
        if run_button:
            ...
            if errors:
                ...
                try:
                    ...
                    Path(out_ptjpl).mkdir(parents=True, exist_ok=True)
                    Path(os.path.dirname(out_meteo)).mkdir(parents=True, exist_ok=True)
                    run_et_calculation(shape_path=shape_path, start_date=start_date_str, end_date=end_date_str, method=method, max_cloud=max_cloud, out_ptjpl=out_ptjpl, out_meteo=out_meteo, progress_callback=update_progress)
                    ...
                    st.success(_("success_message", out_ptjpl=out_ptjpl, out_meteo=out_meteo))
                except Exception as e:
