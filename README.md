...
**Веб-интерфейс модели ML PT-JPL** — приложение на основе Streamlit для оценки фактического эвапотранспирации (ET) сельскохозяйственных полей по данным спутников Landsat и метеорологического реанализа ERA5-Land. Поддерживаются две версии модели:
...
- **График временного ряда** ET, ETc (транспирация), ETs (испарение почвы) и ETr (референсная ET из ERA5-Land)
...
7. **Изучите просмотрщик карт** справа для анализа отдельных каналов и дат.
